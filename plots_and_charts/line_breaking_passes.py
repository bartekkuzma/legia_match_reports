...
class LineBreakingPasses:
    ...
    def prepare_data(self) -> pd.DataFrame:
        ...
        ball_recs = df[
            ...
            & (df.ball_receipt_in_space == True)
            & (df.distance_to_nearest_defender >= 5)
            & (df.x >= 80)
        ]
        # get a list of all related events
        passes_ids = [idx - 1 for idx in ball_recs["index"].to_list()]
        line_breaking_passes_ids = df[
            (df["index"].isin(passes_ids)) & (df["line_breaking_pass"] == True) & (df["pass_outcome"].isna())
        ]["index"].to_list()
    ...
    def plot_line_breaking_passes(
        ...
        sc = pitch.scatter(
            ...
            edgecolors=Constants.COLORS["white"],
            marker="o",
            ax=ax,
        )
        ...
        ax_title = ax.set_title(
            f"{title}'s line-breaking Passes\ninto at least 5m of Space",
            fontsize=26,
            color=Constants.COLORS["white"],
            pad=10,
        )
