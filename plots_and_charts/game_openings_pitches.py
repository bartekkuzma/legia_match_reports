...
class GameOpeningsPitches:
    ...
    def classify_gk_zones(x: float, y: float) -> str:
        ...
        zones = ["P1", "P2", "P3"]
        zone_boundaries = [Constants.PITCH_DIMS["p1"], Constants.PITCH_DIMS["p2"], Constants.PITCH_DIMS["p3"]]
    ...
    def prepare_data(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        goal_kicks.loc[:, "end_zone_gk"] = goal_kicks.apply(
            lambda row: self.classify_gk_zones(row["x_end"], row["y_end"]), axis=1
        )
        ...
        grouped_data = (
            goal_kicks.groupby(["team", "end_zone_gk", "pass_outcome"], dropna=False)
            .count()["id"]
            .to_frame()
            .reset_index()
        )
        pivot_data = grouped_data.pivot_table(
            index=["team", "end_zone_gk"], columns="pass_outcome", values="id", fill_value=0
        )
        pivot_data["ratio"] = pivot_data.apply(
            lambda x: f"{x.get('Complete', 0)}/{x.get('Complete', 0) + x.get('Incomplete', 0)}", axis=1
        )
    ...
    def plot_game_openings(self, team_for: bool, directory: str, figsize: tuple[int, int] = (16, 12)) -> plt.Figure:
        ...
        # Filter data
        data_points_complete = data_points[
            ((data_points["team"] == self.team_for) == team_for) & (data_points["pass_outcome"] == "Complete")
        ].copy()
        data_points_incomplete = data_points[
            ((data_points["team"] == self.team_for) == team_for) & (data_points["pass_outcome"] == "Incomplete")
        ].copy()
        ...
        if not team_for:
            ...
            data_points_incomplete.loc[:, "x_end"] = (
                Constants.PITCH_DIMS["pitch_length"] - data_points_incomplete["x_end"]
            )
            data_points_incomplete.loc[:, "y_end"] = (
                Constants.PITCH_DIMS["pitch_width"] - data_points_incomplete["y_end"]
            )
        ...
        further_y = (
            Constants.PITCH_DIMS["pitch_length"] - Constants.PITCH_DIMS["p3"] + Constants.PITCH_DIMS["p3"] / 2 - 5
            if team_for
            else Constants.PITCH_DIMS["p3"] - Constants.PITCH_DIMS["p3"] / 2 + 5
        )
        ax.text(
    ...
    @staticmethod
    def _generate_pitch_zones(
        team_for: bool, zone_values: dict[str, str], colors: dict[str, str]
    ) -> list[dict[str, any]]:
        """
        ...
        def create_zone(
            x: float, y: float, width: float, height: float, color: str, value: str, name: str, class_: str
        ) -> dict[str, any]:
            return {
        ...
        for i, prefix in enumerate(["P1", "P2", "P3"]):
            if i == 0:
                ...
                y = (
                    ...
                    if team_for
                    else Constants.PITCH_DIMS["pitch_length"] - Constants.PITCH_DIMS["p2"]
                )
            else:
                ...
                y = (
                    ...
                    if team_for
                    else Constants.PITCH_DIMS["pitch_length"] - Constants.PITCH_DIMS["p3"]
                )

            color = ["red", "yellow", "green"][i] if team_for else ["green", "yellow", "red"][i]
            left_side = (
                0 if team_for else Constants.PITCH_DIMS["pitch_width"] - Constants.PITCH_DIMS["penalty_box_left_band"]
            )
            left_half = (
                Constants.PITCH_DIMS["penalty_box_left_band"]
                if team_for
                else Constants.PITCH_DIMS["pitch_width"] - Constants.PITCH_DIMS["six_yard_box"]
            )
            right_side = (
                Constants.PITCH_DIMS["pitch_width"] - Constants.PITCH_DIMS["penalty_box_left_band"] if team_for else 0
            )
            right_half = (
                Constants.PITCH_DIMS["pitch_width"] - Constants.PITCH_DIMS["six_yard_box"]
                if team_for
                else Constants.PITCH_DIMS["penalty_box_left_band"]
            )
            ...
            zones.extend(
                [
                    ...
                    create_zone(
                        left_half,
                        y,
                        Constants.PITCH_DIMS["six_yard_box"] - Constants.PITCH_DIMS["penalty_box_left_band"],
                        height,
                        color,
                        zone_values[f"{prefix} Half Left"],
                        f"{prefix} Left Half-space",
                        prefix,
                    ),
                    create_zone(
                        middle,
                        y,
                        Constants.PITCH_DIMS["pitch_width"] - 2 * Constants.PITCH_DIMS["six_yard_box"],
                        height,
                        color,
                        zone_values[f"{prefix} Central"],
                        f"{prefix} Central",
                        prefix,
                    ),
                    create_zone(
                        ...
                        height,
                        color,
                        zone_values[f"{prefix} Half Right"],
                        f"{prefix} Right Half-space",
                        prefix,
                    ),
                    create_zone(
                        right_side,
