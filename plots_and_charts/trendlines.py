...
class TrendlineCharts:
    ...
    def get_matches_data(full_matches, team_name):
        matches = full_matches[
            ((full_matches["home_team"] == team_name) | (full_matches["away_team"] == team_name))
            & (full_matches["match_status"] == "available")
        ]
        ...
        matches = matches[
            ["match_id", "match_date", "competition", "goals_scored", "goals_conceded", "match_result", "opponent"]
    ...
    def plot_trendline(self, df: pd.DataFrame, title_stat: str, y_stat: str, figsize: tuple[int, int] = (16, 10)):
        ...
        ax.scatter(
            ...
            df["opponent_stat"],
            color=Constants.DARK_BACKGROUND_COLOR,
            edgecolors=Constants.SALMON_COLOR,
            s=100,
            zorder=3,
        )
        # MATCH
        ...
        fig.suptitle(
            f"{self.team_for} vs Opponents", fontsize=36, color=Constants.COLORS["white"], ha="center", y=1.06
        )
        # Set the subtitle
    ...
    def plot_xg_trendline(self, directory: str, figsize: tuple[int, int] = (16, 10)):
        ...
        stat_df = pd.DataFrame({"team_stat": team_stat, "opponent_stat": opponent_stat}).reset_index()
    ...
    def plot_obv_trendline(self, directory: str, figsize: tuple[int, int] = (16, 10)):
        ...
        fig = self.plot_trendline(
            df=data, title_stat="OBV from passes, carries and dribbles", y_stat="On-Ball Value (OBV)", figsize=figsize
        )
