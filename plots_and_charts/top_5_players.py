...
class Top5Players:
    ...
    def prepare_obv_gains_data(self) -> pd.DataFrame:
        obv_events = self.match_events[
            (self.match_events["team"] == self.team_for)
            & (self.match_events["type"].isin(["Pass", "Carry", "Dribble"]))
        ].copy()
        obv_events.loc[obv_events["type"] == "Pass", "pass_outcome"] = obv_events.loc[
            obv_events["type"] == "Pass", "pass_outcome"
        ].fillna("Complete")
        obv_events = obv_events.loc[
            ((obv_events["type"] == "Pass") & (obv_events["pass_outcome"] == "Complete"))
            | ((obv_events["type"] == "Dribble") & (obv_events["dribble_outcome"] == "Complete"))
            | ((obv_events["type"] == "Carry"))
        ]
        ...
        aggregated_df = (
            stats_per_player.groupby("player")
            .agg(
                total_number_of_actions=("number_of_actions", "sum"),
                # total_sum_obv=("sum_obv", "sum"),
                sum_obv_from_pass=(
                    "sum_obv",
                    lambda x: x[stats_per_player["type"] == "Pass"].sum(),
                ),
                sum_obv_from_carry=(
                    "sum_obv",
                    lambda x: x[stats_per_player["type"] == "Carry"].sum(),
                ),
                sum_obv_from_dribble=(
                    "sum_obv",
                    lambda x: x[stats_per_player["type"] == "Dribble"].sum(),
                ),
            )
            .reset_index()
        )
        ...
        aggregated_df["sum_obv_from_dribble"] = aggregated_df["sum_obv_from_dribble"].apply(
            lambda x: x if x >= 0 else 0
        )

        aggregated_df["total_sum_obv"] = (
            aggregated_df["sum_obv_from_pass"]
            + aggregated_df["sum_obv_from_carry"]
            + aggregated_df["sum_obv_from_dribble"]
        )
        # Step 3: Calculate the mean OBV per action
        aggregated_df["mean_obv_per_action"] = (
            aggregated_df["total_sum_obv"] / aggregated_df["total_number_of_actions"]
        )
    ...
    def plot_obv_gain(self, directory: str, figsize: tuple[int, int] = (12, 8)) -> plt.Figure:
        ...
        ax.set_title(
            f"Top 5 for On-ball Value gain \nthrough passed, carries and dribbles",
            color=Constants.COLORS["white"],
            fontsize=24,
            fontweight="bold",
        )
    ...
    def prepare_progressions_data(self) -> pd.DataFrame:
        progressions = self.match_events.loc[
            (self.match_events["team"] == self.team_for) & (self.match_events["type"].isin(["Pass", "Carry"]))
        ]
        progressions.loc[progressions["type"] == "Pass", "pass_outcome"] = progressions.loc[
            progressions["type"] == "Pass", "pass_outcome"
        ].fillna("Complete")
        progressions = progressions.loc[
            ((progressions["type"] == "Pass") & (progressions["pass_outcome"] == "Complete"))
            | ((progressions["type"] == "Carry"))
        ]
        ...
        prog_stats_per_player = prog_grouped_data.agg(number_of_actions=("id", "size")).reset_index()

        # Step 1: Group by player and calculate the sum of all defensive actions
        prog_total_actions_per_player = (
            prog_stats_per_player.groupby("player")
            .agg(total_number_of_actions=("number_of_actions", "sum"))
            .reset_index()
        )
        # Step 2: Pivot the DataFrame to get separate columns for each action type
        prog_pivot_df = (
            prog_stats_per_player.pivot(index="player", columns="type", values="number_of_actions")
            .fillna(0)
            .reset_index()
        )
    ...
    def plot_progressions(self, directory: str, figsize: tuple[int, int] = (12, 8)) -> plt.Figure:
        ...
        prog_final_df = prog_final_df.sort_values(["total_number_of_actions", "Pass", "Carry"], ascending=False)
    ...
    def prepare_defensive_data(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        def_1_events = self.match_events.loc[
            (self.match_events["team"] == self.team_for)
            & (self.match_events["type"].isin(["Ball Recovery", "Block", "Interception", "Clearance"]))
        ]
        def_1_grouped_data = def_1_events.groupby(["player", "type"])
        def_1_stats_per_player = def_1_grouped_data.agg(number_of_actions=("id", "size")).reset_index()

        # Step 1: Group by player and calculate the sum of all defensive actions
        def_1_total_actions_per_player = (
            def_1_stats_per_player.groupby("player")
            .agg(total_number_of_actions=("number_of_actions", "sum"))
            .reset_index()
        )
        # Step 2: Pivot the DataFrame to get separate columns for each action type
        def_1_pivot_df = (
            def_1_stats_per_player.pivot(index="player", columns="type", values="number_of_actions")
            .fillna(0)
            .reset_index()
        )
        # Step 3: Merge the total actions DataFrame with the pivoted DataFrame
        ...
        def_2_events = self.match_events.loc[
            (self.match_events["team"] == self.team_for)
            & (self.match_events["type"].isin(["Foul Committed", "Pressure"]))
        ]
        def_2_grouped_data = def_2_events.groupby(["player", "type"])
        def_2_stats_per_player = def_2_grouped_data.agg(number_of_actions=("id", "size")).reset_index()

        # Step 1: Group by player and calculate the sum of all defensive actions
        def_2_total_actions_per_player = (
            def_2_stats_per_player.groupby("player")
            .agg(total_number_of_actions=("number_of_actions", "sum"))
            .reset_index()
        )
        # Step 2: Pivot the DataFrame to get separate columns for each action type
        def_2_pivot_df = (
            def_2_stats_per_player.pivot(index="player", columns="type", values="number_of_actions")
            .fillna(0)
            .reset_index()
        )
        # Step 3: Merge the total actions DataFrame with the pivoted DataFrame
    ...
    def plot_defensive_1(self, directory: str, figsize: tuple[int, int] = (12, 8)) -> plt.Figure:
        ...
        def_1_final_df = def_1_final_df.sort_values(
            [
        ...
        ax.xaxis.set_major_formatter(FuncFormatter(lambda x, _: f"{int(x)}"))
        ax.set_title(
            f"Top 5 for defensive actions \nrecovery, interception, clearence and block",
            color=Constants.COLORS["white"],
            fontsize=24,
            fontweight="bold",
        )
    ...
    def plot_defensive_2(self, directory: str, figsize: tuple[int, int] = (12, 8)) -> plt.Figure:
        ...
        def_2_final_df = def_2_final_df.sort_values(
            ["total_number_of_actions", "Pressure", "Foul Committed"], ascending=False
        )
        # def threat_creators(ax):
